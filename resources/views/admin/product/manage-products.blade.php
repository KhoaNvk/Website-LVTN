            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="d-flex flex-wrap flex-wrap align-items-center justify-content-between mb-4">
                        ...
                        <a href="{{URL::to('/add-product')}}" class="btn btn-primary add-list"><i
                                class="las la-plus mr-3"></i>Thêm</a>
                    </div>
                </div>
                <div class="col-lg-12">
                            ...
                            @foreach($list_product as $key => $product)
                                <tr>
                                    ...
                                    @if($product->StatusPro == 1)
                                        <td><strong>Đang Hiện</strong></td>
                                    @else
                                        <td><strong>Đang Ẩn</strong></td>
                                    @endif
                                    <td>
                                        <form> @csrf
                                            <div class="d-flex align-items-center list-action">
                                                @if($product->StatusPro == 0)
                                                    <button type="button" class="badge badge-dark mr-2 btn-StatusPro"
                                                            data-toggle="tooltip" data-placement="top" title=""
                                                            data-original-title="Hiện"
                                                            style="border:none;" data-id="{{$product->idProduct}}"
                                                            data-statuspro="1"><i class="fa fa-eye-slash"></i>
                                                    </button>
                                                @else
                                                    <button type="button" class="badge badge-primary mr-2 btn-StatusPro"
                                                            data-toggle="tooltip" data-placement="top" title=""
                                                            data-original-title="Ẩn"
                                                            style="border:none;" data-id="{{$product->idProduct}}"
                                                            data-statuspro="0"><i class="fa fa-eye"></i>
                                                    </button>
                                                @endif
